/*
Emby Server API

Explore the Emby Server API

API version: 4.1.1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package api

import (
	"encoding/json"
)

// checks if the QueryResultSyncModelSyncJobItem type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &QueryResultSyncModelSyncJobItem{}

// QueryResultSyncModelSyncJobItem struct for QueryResultSyncModelSyncJobItem
type QueryResultSyncModelSyncJobItem struct {
	Items []SyncModelSyncJobItem `json:"Items,omitempty"`
	TotalRecordCount *int32 `json:"TotalRecordCount,omitempty"`
}

// NewQueryResultSyncModelSyncJobItem instantiates a new QueryResultSyncModelSyncJobItem object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewQueryResultSyncModelSyncJobItem() *QueryResultSyncModelSyncJobItem {
	this := QueryResultSyncModelSyncJobItem{}
	return &this
}

// NewQueryResultSyncModelSyncJobItemWithDefaults instantiates a new QueryResultSyncModelSyncJobItem object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewQueryResultSyncModelSyncJobItemWithDefaults() *QueryResultSyncModelSyncJobItem {
	this := QueryResultSyncModelSyncJobItem{}
	return &this
}

// GetItems returns the Items field value if set, zero value otherwise.
func (o *QueryResultSyncModelSyncJobItem) GetItems() []SyncModelSyncJobItem {
	if o == nil || IsNil(o.Items) {
		var ret []SyncModelSyncJobItem
		return ret
	}
	return o.Items
}

// GetItemsOk returns a tuple with the Items field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *QueryResultSyncModelSyncJobItem) GetItemsOk() ([]SyncModelSyncJobItem, bool) {
	if o == nil || IsNil(o.Items) {
		return nil, false
	}
	return o.Items, true
}

// HasItems returns a boolean if a field has been set.
func (o *QueryResultSyncModelSyncJobItem) HasItems() bool {
	if o != nil && !IsNil(o.Items) {
		return true
	}

	return false
}

// SetItems gets a reference to the given []SyncModelSyncJobItem and assigns it to the Items field.
func (o *QueryResultSyncModelSyncJobItem) SetItems(v []SyncModelSyncJobItem) {
	o.Items = v
}

// GetTotalRecordCount returns the TotalRecordCount field value if set, zero value otherwise.
func (o *QueryResultSyncModelSyncJobItem) GetTotalRecordCount() int32 {
	if o == nil || IsNil(o.TotalRecordCount) {
		var ret int32
		return ret
	}
	return *o.TotalRecordCount
}

// GetTotalRecordCountOk returns a tuple with the TotalRecordCount field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *QueryResultSyncModelSyncJobItem) GetTotalRecordCountOk() (*int32, bool) {
	if o == nil || IsNil(o.TotalRecordCount) {
		return nil, false
	}
	return o.TotalRecordCount, true
}

// HasTotalRecordCount returns a boolean if a field has been set.
func (o *QueryResultSyncModelSyncJobItem) HasTotalRecordCount() bool {
	if o != nil && !IsNil(o.TotalRecordCount) {
		return true
	}

	return false
}

// SetTotalRecordCount gets a reference to the given int32 and assigns it to the TotalRecordCount field.
func (o *QueryResultSyncModelSyncJobItem) SetTotalRecordCount(v int32) {
	o.TotalRecordCount = &v
}

func (o QueryResultSyncModelSyncJobItem) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o QueryResultSyncModelSyncJobItem) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Items) {
		toSerialize["Items"] = o.Items
	}
	if !IsNil(o.TotalRecordCount) {
		toSerialize["TotalRecordCount"] = o.TotalRecordCount
	}
	return toSerialize, nil
}

type NullableQueryResultSyncModelSyncJobItem struct {
	value *QueryResultSyncModelSyncJobItem
	isSet bool
}

func (v NullableQueryResultSyncModelSyncJobItem) Get() *QueryResultSyncModelSyncJobItem {
	return v.value
}

func (v *NullableQueryResultSyncModelSyncJobItem) Set(val *QueryResultSyncModelSyncJobItem) {
	v.value = val
	v.isSet = true
}

func (v NullableQueryResultSyncModelSyncJobItem) IsSet() bool {
	return v.isSet
}

func (v *NullableQueryResultSyncModelSyncJobItem) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableQueryResultSyncModelSyncJobItem(val *QueryResultSyncModelSyncJobItem) *NullableQueryResultSyncModelSyncJobItem {
	return &NullableQueryResultSyncModelSyncJobItem{value: val, isSet: true}
}

func (v NullableQueryResultSyncModelSyncJobItem) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableQueryResultSyncModelSyncJobItem) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


